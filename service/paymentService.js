...
class PaymentService {
  ...
  async handlePaymentWebhook(webhookData) {
    try {
      const verifyResult = this.payOS.verifyPaymentWebhookData(webhookData);
  ...
  async getPaymentLinkInfo(paymentCode) {
    try {
      const paymentInfo = await this.payOS.getPaymentLinkInformation(paymentCode);
      return paymentInfo;
    } catch (error) {
  ...
  async cancelPaymentLink(paymentCode, reason = "Customer request") {
    try {
      const cancelResult = await this.payOS.cancelPaymentLink(paymentCode, reason);
  ...
  generateUniquePaymentCode() {
    ...
    return Number(timestamp.slice(-6) + randomNum.toString().padStart(3, "0"));
  }
}
